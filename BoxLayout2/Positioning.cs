...
namespace Boxing
{
    public class Position
    {
        ...
        static public void AlignCross (Box parent, List<Box> children, Point position, int usedCrossLength)
        {
            int crossLength;
            ...
            else if (parent.AlignCross == Align.Center)
            {
                for (int i = 0; i < children.Count; i++)
                {
                    ...
                    if (child.ActualSize.GetCross (parent.Orientation) > child.LayoutSize.GetCross (parent.Orientation))
                        crossLength = child.LayoutSize.GetCross (parent.Orientation);
                    else
                        crossLength = child.ActualSize.GetCross (parent.Orientation);
                    child.LayoutPosition.SetCross (parent.Orientation, position.Cross + ((usedCrossLength - crossLength) / 2));
                }
            }
            else if (parent.AlignCross == Align.End)
            {
                for (int i = 0; i < children.Count; i++)
                {
                    ...
                    if (child.ActualSize.GetCross (parent.Orientation) > child.LayoutSize.GetCross (parent.Orientation))
                        ...
                        crossLength = child.ActualSize.GetCross (parent.Orientation);
                    child.LayoutPosition.SetCross (parent.Orientation, position.Cross + (usedCrossLength - crossLength));
                }
            }
